locals {
  ...
  groups = {
    ...
    database-server = {
      count = var.database_count, subnet = "2", group = 0, server_type = var.database_instance_type
    },
    vault = {
      count = var.vault_count, subnet = "3", group = 0, server_type = var.server_instance_type
    },
    client = {
      count = var.client_count, subnet = "4", group = 1, server_type = var.client_instance_type
    },
    observability = {
      count = var.multi_instance_observability ? 4 : 1, subnet = "5", group = 0, server_type = var.observability_instance_type
    }
  }
...
resource "hcloud_server" "server_node" {
  ...
  depends_on = [
    ...
    hcloud_network_subnet.network_subnet["database-server"],
...
resource "hcloud_volume" "database_volumes" {
  for_each = { for entry in var.database_volumes : "${entry.name}" => entry.size }
  location = var.location
  name     = "${each.key}"
  size     = each.value
  format   = "ext4"
...
resource "hcloud_volume_attachment" "database_volumes" {
  for_each = { for index, entry in var.database_volumes : entry.name => entry.server }
  volume_id = hcloud_volume.database_volumes[each.key].id
  server_id = hcloud_server.server_node[each.value].id
  automount = true

  depends_on = [
    hcloud_volume.database_volumes
  ]
}
