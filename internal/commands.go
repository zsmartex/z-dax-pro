...
func Bootstrap(ctx context.Context, config *Config, configPath string) error {
	...
	tf, err := InitTf(ctx, config, os.Stdout, os.Stderr)
	if err != nil {
		return err
	}
	...
	tf, err = InitTf(ctx, config, f, os.Stderr)
	if err != nil {
		return err
	}
	_, err = tf.Output(ctx)
	...
	err = Configure(config, *inv)
	if err != nil {
		return err
	}
	setup := filepath.Join(baseDir, "ansible", "base.yml")
	...
	consulSetup := filepath.Join(baseDir, "ansible", "consul.yml")
	...
	hasBootstrapped, err := BootstrapConsul(config, inv, consul)
	if err != nil {
		return err
	}
	if hasBootstrapped {
	...
	vaultSetup := filepath.Join(baseDir, "ansible", "vault.yml")
	...
	nomadSetup := filepath.Join(baseDir, "ansible", "nomad.yml")
	...
	err = Observability(config, inventory, configPath)
	if err != nil {
		return err
	}
