...
func TestMakeConsulPoliciesAndHashiConfigs(t *testing.T) {
	...
	err = makeConsulPolicies(conf, *inv)
	assert.NoError(t, err)
	...
	inv, err = LoadInventory(filepath.Clean(filepath.Join("testdata", "inventory")))
	assert.NoError(t, err)
	assert.NoError(t, makeConfigs(conf, *inv))
...
func TestMakeSecrets(t *testing.T) {
	...
	assert.Equal(t, 1, strings.Count(secretConf, "CONSUL_GOSSIP_KEY"))
	assert.Equal(t, 1, strings.Count(secretConf, "NOMAD_GOSSIP_KEY"))
