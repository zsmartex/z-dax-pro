...
func unseal(baseDir string, vaultHosts []string, secrets *secretsConfig, init bool) error {
	...
	if !init {
		fmt.Println("Do policy")
		envVars := fmt.Sprintf("export VAULT_SKIP_VERIFY=true && export VAULT_ADDR=https://%s:8200 && ", vaultHosts[0])
		err := runCmd("", fmt.Sprintf("%s vault login %s && vault policy write nomad-server %s/vault/nomad-server-policy.hcl", envVars, secrets.VaultConfig.RootToken, baseDir), os.Stdout)
		if err != nil {
			return err
		}
